  ...
  let response = await getData().then(function (response) {
    response.forEach((element) => {
      $(".apiPlaces").append(
        ...
        <h4>State: ${element.age}</h4>
        <h4>Country: ${element.sex}</h4>
        <h4>Date Visited: ${element.admission}</h4>
        <h4>Rating: ${element.breed}/5 | Review: "${element.review}"</h4>
        ...
        <button class="editPlace" id="${element.animalId},${element.family},${element.age},${element.sex},${element.admission},${element.breed},${element.review},${element.userName}">Edit</button>
        <hr>
        </div>`
      );
    });
